...
const DrAmbedkar = () => {
  const [ambedkarData, setambedkarData] = useState([]);
  const getambedkarData = () => {
    axios.get(`${baseURL}/article`).then((responce) => {
      setambedkarData(responce.data);
    });
  };
  ...
  return (
    <>
      {
        ambedkarData ? ambedkarData.map((item) => (
          <>
           {
            item.articleSection=='ambedkar' ?
