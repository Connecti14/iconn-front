...
const StockMarketContent = () => {
  ...
  const getstockMarketData = () => {
    axios.get(`${baseURL}/article`).then((responce) => {
      setstockMarketData(responce.data);
    });
  };
  ...
  return (
    <>
      {
        stockMarketData ? stockMarketData.map((item) => (
          <>
           {
            item.articleSection=='stock' ?
...
export default StockMarketContent;
